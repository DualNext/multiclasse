...
if uploaded_file is not None:
    ...
    if not st.session_state.show_button:
        ...
        if probabilidade_bru > max(probabilidade_controle, probabilidade_tuberculose):
            cores = ['#800000', 'gray', 'gray']  # Vermelho para Brucelose, Cinza para Controle e Tuberculose
        ...
        for text in ax.texts:
            text.set_fontsize(5)  # Ajuste o valor para o tamanho desejado
    
        ax.set_title('Probabilidades de Diagnóstico', fontsize = 7)
        st.pyplot(fig)

else:
    st.markdown('''<h1 style="color: orange; font-size: 32px;">Diagnóstico de Brucelose e Tuberculose Bovina</h1>''', unsafe_allow_html=True)
    # Subtítulo (h3)
